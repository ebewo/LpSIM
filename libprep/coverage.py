...
def mp_count(pfrags,i):
    ...
    cov_count = sum(list(1 if j[0] <= i <= j[0]+80 else 1 if j[1]-80 <= i <= j[1] else 0 for j in pfrags))
...
def gc_mv_avg(var_dict):
    ...
    for i in xrange(len(var_dict['dna'])-var_dict['window']):
        ...
        GC1 = (window1.count('G') + window1.count('C')) / float(len(window1))*100
        window2 = var_dict['dna'][start2:stop2]
        GC2 = (window2.count('G') + window2.count('C')) / float(len(window2))*100
        window = [GC1,GC2]
        GC = nu.mean(window)
        result.append(GC)
